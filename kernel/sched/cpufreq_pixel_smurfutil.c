...
/* Window size (in ns) */
unsigned int sched_ravg_window2 = MIN_SCHED_RAVG_WINDOW;
...
static bool smugov_update_next_freq(struct smugov_policy *sg_policy, u64 time,
				   unsigned int next_freq)
{
...
static void smugov_fast_switch(struct smugov_policy *sg_policy, u64 time,
			      unsigned int next_freq)
{
	struct cpufreq_policy *policy = sg_policy->policy;

	if (!smugov_update_next_freq(sg_policy, time, next_freq))
		return;
...
static void smugov_deferred_update(struct smugov_policy *sg_policy, u64 time,
				  unsigned int next_freq)
{
	if (!smugov_update_next_freq(sg_policy, time, next_freq))
		return;
				...
				unsigned int prev_freq)
{
	...
	if (curr_ws > (last_ws + sched_ravg_window2)) {
		avg_freq = prev_freq;
		...
		avg_freq /= sched_ravg_window2 / (NSEC_PER_SEC / KHZ);
	}
				...
				unsigned int flags)
{
	...
	if (sg_policy->policy->fast_switch_enabled) {
		smugov_fast_switch(sg_policy, time, next_f);
	} else {
		raw_spin_lock(&sg_policy->update_lock);
		smugov_deferred_update(sg_policy, time, next_f);
	}
...
static int smugov_init(struct cpufreq_policy *policy)
{
	...
	stale_ns = sched_ravg_window2 + (sched_ravg_window2 >> 3);

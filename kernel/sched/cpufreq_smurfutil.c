...
#define DEFAULT_SUSPEND_MAX_FREQ_GOLD 1363200
#define DEFAULT_SUSPEND_CAPACITY_FACTOR 10

#define SMUGOV_KTHREAD_PRIORITY	25

/* Window size (in ns) */
unsigned int sched_ravg_window3 = MIN_SCHED_RAVG_WINDOW;
				...
				unsigned int prev_freq)
{
	...
	if (curr_ws > (last_ws + sched_ravg_window3)) {
		avg_freq = prev_freq;
		...
		avg_freq /= sched_ravg_window3 / (NSEC_PER_SEC / KHZ);
	}
...
static int smugov_init(struct cpufreq_policy *policy)
{
	...
	stale_ns = sched_ravg_window3 + (sched_ravg_window3 >> 3);
